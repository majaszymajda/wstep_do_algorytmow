from random import randint
from time import time
import uuid
import copy
import matplotlib.pyplot as plt

wersja_1_czas = []
wersja_2_czas = []


# tworzenie klasy reperzetujacej wezel listy
class Node():

    def __init__(self, data=None, next_node=None):
        self.data = data
        self.next_node = next_node

    def get_data(self):
        return self.data

    def get_next(self):
        return self.next_node

    def set_next(self, new_next):
        self.next_node = new_next


# tworzenie klasy reprezentującej całą listę
class LinkedList():

    def __init__(self):
        self.start_node = None

    def insert(self, data):
        new_node = Node(data)
        if self.start_node is None:
            self.start_node = new_node
            return
        n = self.start_node
        while n.next_node is not None:
            n = n.next_node
        n.next_node = new_node

    def remove(self, key):
        temp = self.start_node
        if (temp is not None):
            if (temp.data._id == key._id):
                self.start_node = temp.next_node
                temp = None
                return
        while(temp is not None):
            if temp.data._id == key._id:
                break
            prev = temp
            temp = temp.next_node

        if(temp is None):
            return

        prev.next_node = temp.next_node
        temp = None

    def size(self):
        licznik = 0
        if self.start_node is None:
            return licznik
        else:
            n = self.start_node
            while n is not None:
                licznik += 1
                n = n.next_node
        return licznik

    def __iter__(self):
        self.current_node = self.start_node
        return self

    def __next__(self):
        if self.current_node is None:
            raise StopIteration
        else:
            result = self.current_node.data
            self.current_node = self.current_node.next_node
            return result


class Klient:

    typy = ['A', 'B', 'C']
    typ: str
    zajetosc: int

    def __init__(self):
        self._id = uuid.uuid1()
        self.typ = self.typy[randint(0, 2)]
        self.zajetosc = randint(1, 10)

    def __str__(self):
        return f'Klient typ: {self.typ} - zajętość - {self.zajetosc}'


class Okienko:

    nr: int
    typ: str
    zajetosc: int

    _licznik = 1

    def __init__(self, typ):
        self.nr = self._licznik
        Okienko._licznik = self._licznik + 1
        self.typ = typ
        self.zajetosc = 0

    def __str__(self):
        return f'Okienko nr. {self.nr} - {self.typ} - zajętość - {self.zajetosc}'

    def czy_wolne(self):
        return self.zajetosc == 0

    def dodaj_klienta(self, klient):
        self.zajetosc = klient.zajetosc

    def zmniejsz_zajetosc(self):
        if (self.zajetosc > 0):
            self.zajetosc = self.zajetosc - 1


# zad 2 - przypisanie okienku 1 jego typu:  3xA, 3xB, 3xC
okienka_1 = [
    Okienko('A'),
    Okienko('A'),
    Okienko('A'),
    Okienko('B'),
    Okienko('B'),
    Okienko('B'),
    Okienko('C'),
    Okienko('C'),
    Okienko('C')
]
# zad 2 - przypisanie okienku 1 jego typu:  2xA, 2xB, 2xC, 2xE
okienka_2 = [
    Okienko('A'),
    Okienko('A'),
    Okienko('B'),
    Okienko('B'),
    Okienko('C'),
    Okienko('C'),
    Okienko('E'),
    Okienko('E')
]

licznik_2 = 0

for i in range(0, 99):
    # sprawdzanie wolnego okienka dla klienta danego typu
    def znajdz_wolne(klient: Klient, okienka):
        typ = klient.typ
        for okienko in okienka:
            if (okienko.typ == typ or okienko.typ == 'E') and okienko.czy_wolne():
                return okienko
        return None

    def zmniejsz_zajetosci(okienka):
        for okienko in okienka:
            okienko.zmniejsz_zajetosc()

    lista_1 = LinkedList()

    for i in range(100):
        lista_1.insert(Klient())

    lista_2 = copy.copy(lista_1)

    for i in lista_2:
        print(i)
    print(lista_2.size())

    for i in lista_1:
        print(i)
    print(lista_1.size())

    def obslugiwanie_klientow(lista, okienka):
        start_time = time()
        licznik = 0
        while True:
            print('ilosc klientow oczekujacych: ', lista.size())
            if lista.size() == 0:
                break
            licznik += 1
            clients_to_delete = []
            for klient in lista:
                okienko = znajdz_wolne(klient, okienka)
                if okienko:
                    clients_to_delete.append(klient)
                    okienko.dodaj_klienta(klient)
            zmniejsz_zajetosci(okienka)
            for client in clients_to_delete:
                lista.remove(client)
            for okienko in okienka:
                print(okienko)
            # time.sleep(2)
            # print(f'ilosc osob obsluzonych:  {30-lista_1.size()}')
        end_time = time()
        calculated_time = round(abs(start_time - end_time), 2)
        return calculated_time

    czas_1 = obslugiwanie_klientow(lista_1, okienka_1)
    czas_2 = obslugiwanie_klientow(lista_2, okienka_2)
    wersja_1_czas.append(czas_1)
    wersja_2_czas.append(czas_2)
    # print(wersja_1_czas)
    # print(wersja_2_czas)


def wykresy():
    plt.plot(wersja_1_czas)
    plt.plot(wersja_2_czas)
    plt.legend(('wersja bez E - 9 okienek', 'Wersja z E - 8 okienek'))
    plt.xlabel('testy')
    plt.ylabel('czas')
    plt.title('Wykres ')
    plt.grid(True)
    plt.show()


wykresy()

# usprawnieniem jakie mozna wprowadzic jest posortowanie listy klientow
# pod wzgledem typu A, B, C
